...
export class HttpInterceptorService implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    this.systemId = this.currentSystem ? this.currentSystem.id : '';
    if (this.currentUser) {
    ...
    if ( request.body !== null && request.body.hasOwnProperty('system_id')) {
      ...
      request = request.clone({
        body: {...request.body, client_id: this.clientId, token: this.accessToken , system_id: this.systemId}
      });
    ...
    if (request.method === 'GET') {
      ...
      currentParams = currentParams.append('system_id', this.systemId);
